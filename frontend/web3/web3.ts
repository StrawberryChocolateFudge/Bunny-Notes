...
export const ZKBTokenSaleURL_BSC_MAINNET =
  "0xDCA75D59357Cc7d5AAdAfB4b335A7d3ac19b67bC";
...
export const explorerURLWithTxPath: { [key in ChainIds]: string } = {
  [ChainIds.BTT_TESTNET_ID]: `${
    explorerUrl[ChainIds.BTT_TESTNET_ID]
  }${EXPLORERTXPATH.BTT_TESTNET}`,

  [ChainIds.BSC_TESTNET_ID]: `${
    explorerUrl[ChainIds.BSC_TESTNET_ID]
  }${EXPLORERTXPATH.BSC_TESTNET}`,

  // Mainnets
  [ChainIds.BTT_MAINNET_ID]: `${
    explorerUrl[ChainIds.BTT_MAINNET_ID]
  }${EXPLORERTXPATH.BTT_MAINNET}`,

  [ChainIds.ETH_MAINNET]: `${
    explorerUrl[ChainIds.ETH_MAINNET]
  }${EXPLORERTXPATH.ETH_MAINNET}`,
  [ChainIds.BSC_MAINNET]: `${
    explorerUrl[ChainIds.BSC_MAINNET]
  }${EXPLORERTXPATH.BSC_MAINNET}`,
  [ChainIds.POLYGON_MAINNET]: `${
    explorerUrl[ChainIds.POLYGON_MAINNET]
  }${EXPLORERTXPATH.BSC_MAINNET}`,
};
...
function getExplorerURLWithTx(txId: string, network: ChainIds) {
}
...
export async function TESTNETMINTERC20(
  ERC20Contract: any,
  mintTo: string,
  amount: any,
) {
...
export async function depositETH(
  ...
  return await contract.depositEth(commitment, denomination, {
    value: denomination.add(fee),
  });
}
...
export async function estimateWithdraw(
  ...
  let gasConsumed;
  try {
    gasConsumed = await contract.estimateGas.withdraw(
      solidityProof,
      nullifierHash,
      commitment,
      recipient,
...
export function calculateFeeLocally(denomination: string): string {
  ...
  const fee = formatEther(parsedD.div(FEEDIVIDER));
  return fee;
}
