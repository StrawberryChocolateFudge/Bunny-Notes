...
template IsOwner(){
    ...
    // It's purpuse is to calculate the nullifierHash that is unique per transaction!
    signal input salt;
   ...
   // The exra signals to avoid tampering later
    ...
    // Using the same commitment hasher to compute the nullifierHash
    ...
    // The commitment output here is nullifier + salt
