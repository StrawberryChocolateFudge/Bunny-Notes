...
import { createQR, getBundleRootNullifierQRString } from "../qrcode/create";
...
export function downloadA4PDF(
  ...
  doc.setFontSize(10);
  doc.text(
    "To verify the note, open bunnynotes.finance and scan the QR code or paste the code. You can safely share this code with others.",
    3,
    50,
  );
  ...
  doc.setFontSize(18);
  doc.text(
    "Use the QR Code to withdraw the value. Keep this confidential",
    2,
    235,
  );
...
export const Terms =
  `The following terms and conditions apply to the use of Bunny Notes on the website bunnynotes.finance:\n
...
export interface DownloadBunnyBundlePDFProps {
  bearerText: string;
  bundleValue: string;
  bundleSize: number;
  cardType: string;
...
export type BundleData = {
  ...
  dataURL: string;
  bunnyNote: string;
  verifierQRString: string;
  verifierQRCode: string;
};

export const downloadBunnyBundlePDF = async (
  {
    bearerText,
    bundleValue,
    bundleSize,
    ...
    tokenAddress,
    bundle,
  }: DownloadBunnyBundlePDFProps,
) => {
  let doc = new jsPDF();

  const bunnyBundleNotes = bundle.bunnyBundle;

  for (let i = 0; i < bundleSize; i++) {
    const note = bunnyBundleNotes[i];

    const parsedBunnyBundle = await parseBundleNote(note);

    const verifierQRString = await getBundleRootNullifierQRString(
      parsedBunnyBundle.amount,
      parsedBunnyBundle.currency,
      parsedBunnyBundle.netId.toString(),
      bundle.root,
      toNoteHex(parsedBunnyBundle.deposit.nullifierHash),
    );

    const dataUrl = await createQR(note);
    const verifierDataUrl = await createQR(verifierQRString);
    await createPage(
      i === 0,
      doc,
      networkName,
  ...
  // Add the contract under it

  //Then add a new page and render stuff on it

  doc.save(`BunnyBundle-${bundleValue}-${bundle.root}.pdf`);
};
...
function createPage(
  ...
  //TODO: The first page is good, the rest renders with grey text only!
  //Need to fix that!
