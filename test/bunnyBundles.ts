...
describe("Bunny bundles", function () {
    ...
    it("Deploy a bunny bundle contract and deposit ETH for a bundle and withdraw!", async function () {
        ...
        const bundleSize = 5;
        const { bunnyBundle, leaves, root, tree } = await createBundle({ currency: "ETH", totalValue, netId: 1337, size: bundleSize });
        const noteValue = await bunnyBundles.getNoteValue(parseEther(totalValue), bundleSize);
        ...
        expect(await (await bunnyBundles.bundles(rootHex)).used).to.equal(false);

        expect(await bunnyBundles.nullifierHashes(rootHex, nullifierHash)).to.equal(false);
        ...
        expect(await provider.getBalance(bunnyBundles.address)).to.equal(parseEther("0"));

        const depositEthTx = await bunnyBundles.connect(alice).depositEth(rootHex, parseEther(totalValue), bundleSize, { value: valueWithFee });
        ...
        expect(await provider.getBalance(bunnyBundles.address)).to.equal(parseEther(totalValue));
        ...
        const merkleProof = generateMerkleProof(parsed.deposit.commitment, leaves) as MerkleProof;
        ...
        expect(await bob.getBalance()).to.equal(bobBalanceBeforeWithdraw.add(noteValue));
        ...
        expect(bundleAfterWithdraw.valueLeft).to.equal(parseEther(totalValue).sub(noteValue));
    })
    ...
    it("Create a bunny bundle that has only 1 note, deposit ETH and withdraw", async function () {
        ...
        await bunnyBundles.connect(alice).depositEth(toNoteHex(root), parseEther(totalValue), bundleSize, { value: parseEther(totalValue).add(fee) });
        ...
        expect(errorOccured).to.equal(true);
        expect(errorMessage.includes("Root hash used")).to.be.true;

    })

    it("Create a bunny bundle and deposit tokens then withdraw.", async function () {
        const {
            owner,
            alice,
            bob,
        ...
        const bundleSize = 10;
        const { bunnyBundle, leaves, root, tree } = await createBundle({ currency: "USTM", totalValue, netId: 1337, size: bundleSize });
        const noteToDeposit = bunnyBundle[0]; // I use the first from the bundle
        const parsed = await parseBundleNote(noteToDeposit);
        ...
        expect(await USDTM.allowance(alice.address, bunnyBundles.address)).to.equal(toApprove);
        ...
        expect(await USDTM.balanceOf(bunnyBundles.address)).to.equal(parseEther(totalValue));
        ...
        expect(await USDTM.balanceOf(bob.address)).to.equal(bobBalanceBeforeWithdraw.add(noteValue));
        expect(await USDTM.balanceOf(bunnyBundles.address)).to.equal(parseEther(totalValue).sub(noteValue));
    ...
    it("Test feeless tokens", async function () {
        ...
        const bundleSize = 10;
        const { bunnyBundle, leaves, root, tree } = await createBundle({ currency: "USDTM", totalValue, netId: 1337, size: bundleSize });
        const noteValue = await bunnyBundles.getNoteValue(parseEther(totalValue), bundleSize);
        ...
        expect(await USDTM.balanceOf(bunnyBundles.address)).to.equal(parseEther("1"));
        expect(await USDTM.balanceOf(owner.address)).to.equal(parseEther("0"));
    })
