...
describe("Bunny Notes", async function () {
  it("Should deploy a bunny notes and create an ETH note", async function () {
    ...
    const noteString1 = await deposit({
      currency: "ETH",
      amount: "100",
      netId: 1337,
    });
    ...
    const cs = await bunnyNotes.commitments(
      toNoteHex(parsedNote.deposit.commitment),
    );
  ...
  it("Should create a ERC-20 Note", async function () {
    ...
    expect(errorOccured).to.be.true;
    expect(errorMessage.includes("ERC20: transfer amount exceeds balance")).to
      .be.true;
  ...
  it("alice creates a ETH note and bob address withdraws it", async function () {
    ...
    const nullified = await bunnyNotes.nullifierHashes(
      toNoteHex(parsedNote.deposit.nullifierHash),
    );
  ...
  it("Testing Feeless token", async function () {
    ...
    const noteString = await deposit({
      currency: "ZKB",
      amount: "100",
      netId: 1337,
    });
    ...
    await feelesstoken.connect(bob).approve(
      bunnyNotes.address,
