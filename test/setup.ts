...
export async function setUpBunnyNotes(): Promise<
  {
...
> {
  ...
  const VerifierFactory = await ethers.getContractFactory(
    "contracts/WithdrawVerifier.sol:Verifier",
  );
  ...
  const bunnyNotesDeploy = await BunnyNotesFactory.deploy(
    Verifier.address,
  ...
  const tokensaleDeploy = await TokenSaleFactory.deploy(
    relayer.address,
    USDTM.address,
  );
...
export async function setUpBunnyBundles(): Promise<SetUpBundlesType> {
  ...
  const bunnyBundlesDeploy = await BunnyBundlesFactory.deploy(
    Verifier.address,
    feelesstoken.address,
  );
